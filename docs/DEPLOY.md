...
- **배포가 느리면 남아 있는 실행부터 본다.** 큐에 걸린 채 상태가 깨진 실행은
  ...
  워크플로를 의심하기 전에 아래로 끝나지 않은 실행을 확인한다

  ```bash
  gh run list --limit 40 --json databaseId,status,name \
    --jq '.[] | select(.status != "completed")'
  ```

  `gh run list` 를 기본값으로 보면 최근 몇 건만 나와서, 오래된 유령이 아래로 밀려
  안 보인다. 상태로 걸러야 한다.

  **자리가 풀려도 실행 기록은 남는다.** `queued` 인 채 `updated_at` 이 멈춘 상태로
  목록에 계속 보인다. 새 배포를 막지는 않으므로 지우려면 GitHub 지원에 문의한다
